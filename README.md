...
First, backup any existing files that exist so this doesn't overwrite your work.
...
The prompt is meant to be simple while still providing a lot of information to the user, particularly about the status of the git project, if the PWD is a git project. This prompt sets `PROMPT` and `RPROMPT`. The `RPROMPT` shows the git and suspended jobs info. The main symbol used on the actual prompt line is `❯`.
